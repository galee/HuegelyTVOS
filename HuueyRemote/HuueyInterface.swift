...
public class HuueyInterface {
    ...
    /**
     ...
     - Parameter: light, HuueyLight: Light thats brightness should be changed
     */
    public func setHue(hue: Int, light: HuueyLight) {
        let data = ["hue":hue]
        self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)
    }
    ...
    public func set(light: HuueyLight) {
        
    }
    
    public func setXY(light:HuueyLight) {
        let data = ["xy":light.getXY()]
        self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)
    }

    public func setCT(light:HuueyLight) {
        let data = ["ct":light.getCT()]
        self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)
    }

    public func setHSB(light:HuueyLight) {
        var data = ["hue":light.getHue(), "sat":light.getSaturation(),"bri":light.getBrightness()]
    ...
    public func sync(light:HuueyLight) {
        if  light.getStateChangeSet().contains("xy") {
            let data = ["xy":light.getXY()]
            self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)
            
        } else if  light.getStateChangeSet().contains("ct") {
            let data = ["ct":light.getCT()]
            self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)
            
        } else if  light.getStateChangeSet().contains("sat") {
            let data = ["sat":light.getSaturation()]
            self.request(generateUrl(HuueyGet.Light, id: light.getId()) + "/state", method: HuueyMethods.PUT, data: data)

        }
        light.resetStateChange()
